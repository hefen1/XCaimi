...
public:
	...
	virtual void DrawCurve(DWORD linetype,
		CTValues<DrawPointInfo> pRefX,
		CTValues<DrawIndicatrixData> pRefValues,
					   ...
					   COLORREF cr = 0, int Index = 0,const char* Name = NULL)
{
	...
	DrawPointInfo*		       pXPoint = pRefX.m_pValue;
	DrawIndicatrixData*	   pValues = pRefValues.m_pValue;
	...
	int nFormerPos = -1;
	int nCurPos;
	double LastVal = (dMinValue + dMaxValue) / 1;

	for (int i = 0; i < pRefValues.m_lSize; i++)
	{
		if(pValues[i].m_dValue == hx_DefMinValues)
		{
			nFormerPos = -1;
			continue;
		}
		if( (dMaxValue - dMinValue) == 0 )
			break;

		if( !IsValue( &pValues[i].m_dValue,NULL ) )
			continue;

		nCurPos = (int)((float)m_PY * (((double)(pValues[i].m_dValue ? pValues[i].m_dValue : LastVal) - dMinValue) / (dMaxValue - dMinValue)) /*+ 0.5*/);		
		...
		{    		
			if (nFormerPos < 0)
			{
				nFormerPos = (int)((double)m_PY * 
					(((double)LastVal/*pValues[i - 1].m_lValue*/ - dMinValue) / (dMaxValue - dMinValue)) /*+ 0.5*/);				
			}
			if(i == 1)
			{
		...
		if(pValues[i].m_dValue)
			LastVal = pValues[i].m_dValue;

...
int main ()
{
   ...
   if (XDp_doc -> LoadFile ("TradeConfig.xml"))
   {
	   TiXmlNode* pNode1 = NULL;
	   TiXmlNode* pNode = NULL;
	   TiXmlElement* pElement = XDp_doc -> RootElement();	

	   pNode = TinyXPath::XNp_xpath_node(pElement, "//CellConfig");

	   pNode1 = TinyXPath::XNp_xpath_node(pNode, "Cell[@id=1]");
	   int iCount = TinyXPath::i_xpath_int(pNode1,"count(//Cell)");
	   printf ("-- Tables : [%d] --\n", iCount);
	   pNode = pNode1->FirstChildElement("ModifyPassWordType");
	   iCount = TinyXPath::i_xpath_int(pNode1,"count(//item)");
	    printf ("-- Tables : [%d] --\n", iCount);
   }

   if (XDp_doc -> LoadFile ("Tables.xml"))
   {
	  int iCount = 0;
	  TiXmlNode* pNode1 = NULL;
	  TiXmlNode* pNode = NULL;
	  ...
	  pNode = TinyXPath::XNp_xpath_node(pElement, "//Tables");
	  if (pNode)
	  {
		  // S_res = TinyXPath::S_xpath_string (pNode->Parent(), "@Name");
		  //printf ("-- Tables : [%s] --\n", S_res.c_str());


		  iCount = TinyXPath::i_xpath_int(pNode,"count(//item)");
		  printf ("-- Tables : [%d] --\n", iCount);

	  }

	  iCount = TinyXPath::i_xpath_int(pElement,"count(//Tables/item[@Name='2501']/column)");
	  printf ("-- Tables : [%d] --\n", iCount);

	  
	  pNode = TinyXPath::XNp_xpath_node(pElement, "//Tables/item[@Name='2501']");
	  if (pNode)
	  {
		 // S_res = TinyXPath::S_xpath_string (pNode->Parent(), "@Name");
		  //printf ("-- Tables : [%s] --\n", S_res.c_str());
		  ...
		  iCount = TinyXPath::i_xpath_int(pNode,"count(//column)");
		  printf ("-- Tables : [%d] --\n", iCount);

		  pNode1 = TinyXPath::XNp_xpath_node(pNode, "column[9]");
		  if (pNode1)
		  {
			  S_res = TinyXPath::S_xpath_string (pNode1, "@DispName");
			  printf ("-- Tables : [%s] --\n", S_res.c_str());
		  }	
	  }

	  pNode = TinyXPath::XNp_xpath_node(pElement, "//ReqParam/item[@Name='3010']");
	  if (pNode)
	  {
		  // S_res = TinyXPath::S_xpath_string (pNode->Parent(), "@Name");
		  //printf ("-- Tables : [%s] --\n", S_res.c_str());
		  ...
		  iCount = TinyXPath::i_xpath_int(pNode,"count(//param)");
		  printf ("-- Tables : [%d] --\n", iCount);
	  }
	  ...
	  if (pNode)
	  {
		  S_res = TinyXPath::S_xpath_string (pNode, "/*[1]/@Name");
		  printf ("-- Tables : [%s] --\n", S_res.c_str());

		  S_res = TinyXPath::S_xpath_string (pNode, "/*[1]/@DispName");
		  printf ("-- Tables : [%s] --\n", S_res.c_str());

		  iCount = TinyXPath::i_xpath_int(pNode,"count(/column)");
		  //S_res = TinyXPath::S_xpath_string (pNode, "count(/column)");
		  printf ("-- Tables : [%d] --\n", iCount);
	  }
	  pNode1 = TinyXPath::XNp_xpath_node(pNode, "/*[1]");
	  if (pNode1)
	  {
		  S_res = TinyXPath::S_xpath_string (pNode1, "@Name");
		  printf ("-- Tables Item : [%s] --\n", S_res.c_str());
	  }

	   pNode1 = TinyXPath::XNp_xpath_node(pNode, "/column[@Name='stock_code']");
	  if (pNode1)
	  {
		  S_res = TinyXPath::S_xpath_string (pNode1, "@Name");
		  printf ("-- Tables Item : [%s] --\n", S_res.c_str());
   ...
   if (XDp_doc -> LoadFile ("openoff.xml"))
   {
	  ...
	  printf ("-- openoff : [%s] --\n", S_res.c_str());
      S_res = TinyXPath::S_xpath_string (XDp_doc -> RootElement (), "//text:sequence-decls/*[2]/@text:name");
	  printf ("-- openoff : [%s] --\n", S_res.c_str());
   }   
   delete XDp_doc;

...
void CTradeQueryBaseDlg::DoGridDBClick()
{
	...
	if ((iRow > 0) && (m_pGrid->GetRowCount() > 1))
	{
		...
		if (pParam)
		{
			...
			CString sFundCode = pItem->GetStrByName(TRADE_PARAM_FUNDCODE);
			sFundCode.Trim();
			if (!sFundCode.IsEmpty())
			{
				CLinkageParam param(TRUE);
				param.AddParam(TRADE_PARAM_FUNDCODE, sFundCode);
				SendLinkageMsg(TRADE_MESSAGE_FUNDCODE,&param);
			}
		}
	}
}
...
{
	CTradeDialogBase::DoDataExchange(pDX);
	//if (m_pGrid)
	//{
...
void CTradeQueryBaseDlg::OpenLayOutMsg(CString sLayout)
{
	...
	if (iRow > 0)
	{
		...
		{
			IDataItem* pItem = (IDataItem*)pParam;
			CString sStockCode = pItem->GetStrByName(FUNDCODE);
			if (!sStockCode.IsEmpty())
			{
				param.AddParam(_T(FUNDCODE), sStockCode);

...
{
	CTechKLineImp* pKLine = GetPKLine();
	if( pKLine == NULL )
		return;

	CManagerBase* pMgr = pKLine->GetTechLineMgr();
	if (pMgr != NULL)
	{
		int nType = 3;
		pMgr->GetTimeByPos(nPos,strBuf,(bLast && nType == 3)?4:nType);
	}
}
...
void CGraphics::YAxis()
{
	...
	if( pPKline != NULL && !pPKline->IsFenshiDrawType() ) // 计算坐标文本是否超过显示边界
	{
		...
		if(nCharCount > 0) 
		{
			nLeftWidth  = (nCharWidths - m_rcDefMargin.left.m_nDef);
			nCharWidths = nCharWidths / nCharCount;
			/*if(CDrawTechKLine::bIsVol) //qinhn test 20090216修改让K线图界面，VOL栏坐标最多显示五位数
			{	
				if(nCharCount>5)
					dMulti = nCharCount - 5;
				dMulti = pow((int)10,(int)dMulti);
...
int CGraphics::DrawVarTitle(CArray< CToptextType<double>*,CToptextType<double>* >& Value, int nStartPositon/* = 0*/)
{
	try
	{
		...
		SetStringAlign(DT_LEFT, DT_VCENTER);
		CTechKLineImp* pImpOld = NULL;
		COLORREF clrOld = RGB(0, 0, 0);
		for(int i = 0; i < nCount; i++ )
		{				
			int nStartX = pt.x;
			int nStartY = m_Rect.top;
			pValue = Value.GetAt(i);
			CTechKLineImp* pImpCur = pValue->m_pTechLine;
			BOOL bAdd = (pImpCur == pImpOld);
			COLORREF clrOldCur = m_pDC->SetTextColor(pValue->m_cl);
			if(clrOld == RGB(0, 0, 0))
			{
				clrOld = clrOldCur;
			}
			...
			size.cx += 4;
			if(pValue->m_nImageIndex >= 0)	//有图片
				size.cx += 16;

			if( pt.x + size.cx + 16 > rect.right )
			{
				pImpCur->SetVarTitleRect(CRect(nStartX, nStartY, rect.right, nStartY + size.cy), bAdd);
				break;
			}
			...
			if(pValue->m_nImageIndex >= 0)	//有图片
			{
				g_hsGraphicsData.m_expressImages->Draw(m_pDC, pValue->m_nImageIndex, CPoint(pt.x, m_Rect.top), ILD_TRANSPARENT);
				pt.x += 16;
			}
			...
			pImpCur->SetVarTitleRect(CRect(nStartX, nStartY, pt.x, nStartY + size.cy), bAdd);
			pImpOld = pValue->m_pTechLine;
		}
		m_pDC->SetTextColor(clrOld);
		m_pDC->SelectObject(hOldFont);
...
CTechKLineImp* CGraphics::GetPKLine()
{
	return (CTechKLineImp*)m_pThis; // 一定是它
}

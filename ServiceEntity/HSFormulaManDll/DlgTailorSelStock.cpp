// DlgTailorSelStock.cpp : implementation file added by HS
//
...
IMPLEMENT_DYNAMIC(CDlgTailorSelStock, CDialog)
CDlgTailorSelStock::CDlgTailorSelStock(CWnd* pParent /*=NULL*/,
									   HSOpenExpressUseData* pData/* = NULL*/,
									   long* pThis /*= NULL*/)
									   : CDialog(CDlgTailorSelStock::IDD, pParent)
...
// 	m_cDeleteThis = 2;
	...
	if( pThis )
	{
		CDialog::Create(CDlgTailorSelStock::IDD,pParent);
		CenterWindow(CWnd::FromHandle(g_hParentWnd));
	}
}

CDlgTailorSelStock::~CDlgTailorSelStock()
{
}

void CDlgTailorSelStock::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDITTOTALSTOCK, m_wndTotalStock);
	DDX_Control(pDX, IDC_EDITSELECTSTOCK, m_wndSelectStock);
	DDX_Control(pDX, IDD_INCLUDE, m_wndIncludeStock);
	DDX_Check(pDX, IDC_CHECK_CHUQUAN, m_bCheckChuQuan);
}


BEGIN_MESSAGE_MAP(CDlgTailorSelStock, CDialog)
	//{{AFX_MSG_MAP(CDlgTailorSelStock)
	ON_WM_SIZE()
...
BOOL CDlgTailorSelStock::OnInitDialog() 
{
...
void CDlgTailorSelStock::Init(int nCopy/* = CTreeCtrlFormula::CopyDelete*/)
{
	...
	if( pCurTree )
	{
		...
		CTreeCtrlFormula* pTree = pDlg->GetCurTree( (WORD)MoreKLine );
		...
		InitCWData(GB_GUBEN_NAME);
		InitCWData(Cj_GUBEN_NAME);
...
/*void CDlgTailorSelStock::OnSize(UINT nType, int cx, int cy) 
{
...
void CDlgTailorSelStock::SetSize()
{
CWnd* pWnd;
CRect rect,rc,rc1;
...
*/
void CDlgTailorSelStock::OnAdvance() 
{
...
void CDlgTailorSelStock::OnCancel() 
{
...
void CDlgTailorSelStock::OnUsernotes() 
{
...
//要改动
void CDlgTailorSelStock::OnOK() 
{
	m_strBlock.Empty();
	Select();
}

int CDlgTailorSelStock::Select() 
{
	...
	*/	
	...
	m_pHSUnion.Copy(pHSUnion);  //就在此处改
	...
	*/
		...
		else
		{
			...
			else
			{
				...
				if(hItem)
				{
					...
					if( pTreeGroup )
					{
						if( pTreeGroup->IsStatus(hx_OtherData) )
						{
							m_sSingleSel.m_strCondition.Format("%i",pTreeGroup->m_dwID);
						}
					}
				}
				else
	...
	/*
	if( (m_curSelStockCondition.m_dStyle & 0x0F00) & SelectStock_Union )
	{
	if( m_pHSUnion.m_aySub )
	{
	CTime baseTM = CTime(m_curSelStockCondition.m_lEndDate / 10000,
	(m_curSelStockCondition.m_lEndDate % 10000)/100,
	(m_curSelStockCondition.m_lEndDate % 10000)%100,0,0,0);

	CString curEndDate;
	UnionSelConditionSub* pCurData;
	for(int i = 1; i < m_pHSUnion.m_aySub->GetSize(); i++)
	{
	pCurData = m_pHSUnion.m_aySub->GetAt(i);
	pCurData->m_lDateCount = pCurData->m_nEndDate - pCurData->m_nBeginDate;

	CTime timeDest = baseTM - CTimeSpan(pCurData->m_nBeginDate,0,0,0);
	curEndDate.Format("%04d%02d%02d",timeDest.GetYear(),
	timeDest.GetMonth(),timeDest.GetDay());
	pCurData->m_nCalcEndDate = atol(curEndDate);
	}
	}
	}
	*/
	...
	// 执行
	long lRet = DoSelectStock(&m_curSelStockCondition, HS_SELECTSTOCK_DingZhi);//::SendMessage(g_hParentWnd,HX_USER_COMPILEDATA,HS_SELECTSTOCK_DingZhi,
...
int CDlgTailorSelStock::DoSelectStock( SelStockCondition* pSelStockCondition ,int style)
{
...
void CDlgTailorSelStock::OnToblock() 
{
	// TODO: Add your control notification handler code here
	::SendMessage(g_hParentWnd,HX_USER_COMPILEDATA,1,(LPARAM)this);
}

long CDlgTailorSelStock::LoadCurExp(WPARAM wParam /*= 0*/, LPARAM lParam /*= 0*/)
{	
		...
		||wParam== TEMPUNION_DELUNION)
	{
		...
		CTreeGroup* pTreeGroup = (CTreeGroup*)pCurTree->GetItemData(pCurTree->GetSelectedItem());
		if( wParam == TEMPUNION_NEWCONDITION)
		{
			CDlgUnionList* pTempUnion = (CDlgUnionList*)lParam;
			if ( pTempUnion == NULL) return 0;
			//是基本面资料
			if( pTreeGroup != NULL && pTreeGroup->IsStatus(hx_OtherData) )
			{
				m_sSingleSel.m_strCondition.Format("%i",pTreeGroup->m_dwID);
				m_sSingleSel.m_SubData.m_cType = m_wndCompare.GetCurSel();
				m_wndData1.GetWindowText(m_sSingleSel.m_SubData.m_dData1);
	...
	else if( wParam == CTreeCtrlFormula::HSBrowser )
	{
			...
			/*!strItem.CompareNoCase(CTreeCtrlFormula::g_strJiben)*/ )
		{
			if( pGroup->m_dwID == JiBenMian )
			{
				...
				InitCondition(&m_wndCompare,4);
				*/}
				if( (pGroup->m_dwID >= COLUMN_CAPITALIZATION_BEGIN && pGroup->m_dwID < COLUMN_CAPITALIZATION_END ) || // 股本数据
					(pGroup->m_dwID >= COLUMN_FINANCE_BEGIN && pGroup->m_dwID < COLUMN_FINANCE_END ) )
				{
	...
	else if( wParam == HX_EXPRESSSENTENCE )
	{
		...
		switch ( m_sSingleSel.m_nType )
		{
			...
			m_wndConNotes.ShowWindow(SW_SHOW);
			m_wndConNotes.SetWindowText("指标线");
...
BOOL CDlgTailorSelStock::ShowWindow( int nCmdShow )
{
...
CString CDlgTailorSelStock::GetNameByKey(CString& strFindKeyName) 
{
	CString strRet;
	if( g_pTableHeadData )
	{
...
BOOL CDlgTailorSelStock::SaveLoad(UINT nMode,CString strFileName /*= ""*/,DWORD dStyle /*= 0*/)
{
	/*if( !IsType(Condition) )
	return 0;
	*/
...
void CDlgTailorSelStock::Serialize(CArchive& ar,int& nCountRef)
{
	nCountRef = 0;
...
void CDlgTailorSelStock::OnButexpress() 
{
	// TODO: Add your control notification handler code here
	CTreeCtrlFormula* pCurTree = m_wndTab.GetTree();
	if( pCurTree )
...
void CDlgTailorSelStock::OnButparam() 
{
	// TODO: Add your control notification handler code here
	CDlgFormulaMan* pParentDlg  = (CDlgFormulaMan*)this->GetParent();
	if( pParentDlg == NULL )
...
void CDlgTailorSelStock::MyNcDestroy()
{
	::SendMessage(g_hParentWnd,HX_USER_COMPILEDATA,20,0);
}

void CDlgTailorSelStock::Delete()
{
	SaveLoad(CArchive::store);
	...
	if( pCurTree != NULL )
	{
		...
		if( hRootItem != NULL )
		{
			...
			while(hChildItem != NULL)
			{
				pTreeGroup = (CTreeGroup*)pCurTree->GetItemData( hChildItem );
				if( pTreeGroup )
				{
					delete pTreeGroup;
				}
...
void CDlgTailorSelStock::OnBnClickedClose()
{
	// TODO: Add your control notification handler code here
}

void CDlgTailorSelStock::InitCWData(/*CComboBox* pBox,*/CString strName) 
{
	if( g_pTableHeadData )
	{
		void* pValue;
			...
			if( !pMap->IsEmpty() )
			{
				...
				while(pos)
				{
					...
					if(pValue != NULL)
					{
						...
						pTreeGroup = new CTreeGroup(hx_OtherData,(DWORD)wKey);
						pCurTree->SetItemData(hItem,(DWORD)pTreeGroup);

						//		nIndex = pBox->AddString(pCol->m_strName);
						//		if( nIndex != -1 )
						//		{
...
void CDlgTailorSelStock::OnSelchangeCompare()
{
	// TODO: Add your control notification handler code here
	...
	case FormulaCompareSubData::JieYu:		//	介于
		...
		{
		m_wndAndOr.ShowWindow(SW_SHOW);
		m_wndAndOr.SetWindowText("周期后上穿");
		m_wndData2.ShowWindow(SW_SHOW);
		m_wndMiddle.ShowWindow(SW_HIDE);
		}
		break;
		...
		{
		m_wndAndOr.ShowWindow(SW_SHOW);
		m_wndAndOr.SetWindowText("周期后下破");
		m_wndData2.ShowWindow(SW_SHOW);
		m_wndMiddle.ShowWindow(SW_HIDE);
		}
		break;
...
void CDlgTailorSelStock::InitCondition(CComboBox* pBox,int nType)
{
	pBox->ResetContent();
	int nCount;
...
LRESULT CDlgTailorSelStock::OnExchangeData(WPARAM wp,LPARAM lp)
{
	if( wp == 1 ) // 股票列表
	{		
	}
...
void CDlgTailorSelStock::OnBnClickedButfind()
{
	// TODO: Add your control notification handler code here
	DWORD dwMask = 0;
	dwMask = MoreKLine;
...
LRESULT CDlgTailorSelStock::OnUserDragData(WPARAM wParam, LPARAM lParam)
{
...
void CDlgTailorSelStock::OnSelchangeTab(NMHDR* pNMHDR, LRESULT* pResult) 
{

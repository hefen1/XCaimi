...
const int WM_WINMGR = RegisterWindowMessage(_T("WM_WINMGR"));

CWinMgr::CWinMgr(WINRECT* pWinMap,BOOL bInit) : m_map(pWinMap)
{
	if(bInit)
...
void CWinMgr::OnGetSizeInfo(SIZEINFO& szi, WINRECT* wrc, CWnd* pWnd)
{
	...
	if (wrc->IsGroup()) {
		...
		//20090813 add by fangly being:增加对普通图元的处理
		if(wrc->IsChart() && wrc->Type())
		{
			szi.szAvail = rcParent.Size();
			SendGetSizeInfo2(szi, pWnd, wrc->RectInfo());	
		}
		//end
...
BOOL CWinMgr::SendGetSizeInfo2(SIZEINFO& szi, CWnd* pWnd, void* info)
{
	NMWINMGR nmw;
	nmw.code = NMWINMGR::GET_CHART_SIZEINFO;	// request size info
	nmw.idFrom = (UINT_PTR)info;				// ID of chart I'm computing
	nmw.sizeinfo = szi;					// copy

	pWnd->SendMessage(WM_WINMGR, (WPARAM)info, (LPARAM)&nmw);

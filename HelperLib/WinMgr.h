...
protected:
	...
	void*   info;           // rect info 20090813 add by fangly

public:
	WINRECT(WORD f, UINT id, LONG p,void* info = 0);
	...
	//20090813 add by fangly begin:增加图元信息接口函数
	void* RectInfo() { return info; }
	BOOL  IsChart() {return info != 0;}
	//end
};
...
public:
	CWinMgr(WINRECT* map,BOOL bInit = TRUE);
	virtual ~CWinMgr();
...
protected:
	...
	BOOL SendGetSizeInfo2(SIZEINFO& szi, CWnd* pWnd, void* info);

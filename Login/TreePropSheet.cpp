...
void CTreePropSheet::SetTreeImages(CImageList *pImages)
{
	if (m_tabDefaultImages.GetSafeHandle())
		m_tabDefaultImages.DeleteImageList();
	m_tabDefaultImages.Create(pImages);	
}
...
void CTreePropSheet::RefillPageTree()
{
	...
	CTabCtrl	*pTabCtrl = (CTabCtrl*)GetTabControl();
	...
	for (int nPage = 0; nPage < nPageCount; ++nPage)
	{
		...
		if (hItem)
		{
			...
			if (m_bTreeImages)
			{
				...
				m_pwndPageTree->SetItemImage(hItem, nPage,nPage);//nImage, nImage);
			}
		}
	}
}
...
{ 
	...
	pTab->SetImageList(&m_tabDefaultImages);
	...
	// MFC7-support here (Thanks to Rainer Wollgarten)
	{
		...
		m_pwndPageTree->CreateEx(
			...
			WS_TABSTOP|WS_CHILD|WS_VISIBLE|WS_BORDER|dwTreeStyle, 
			rectTree, this, s_unPageTreeId);
	}
	
	if (m_bTreeImages)
	{
		m_pwndPageTree->SetImageList(&m_tabDefaultImages/*&m_Images*/, TVSIL_NORMAL);
		m_pwndPageTree->SetImageList(&m_tabDefaultImages/*&m_Images*/, TVSIL_STATE);
	}
	...
	if(pButton)
	{
		pButton->SetWindowText(_T("确定"));
	}
	pButton = GetDlgItem(IDCANCEL);
	if(pButton)
	{
		pButton->SetWindowText(_T("取消"));
	}
	return bResult;
}
